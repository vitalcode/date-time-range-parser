...
import uk.vitalcode.dateparser.token.{Date, DateRange, Day, Month, Range, TimeRange, Time, Token, WeekDay, Year}

object DateTokenAggregator {

  def indexTokenList(tokens: List[Token]) = {
  ...
  def aggregate(list: List[Token]): List[Token] = {
    list match {
