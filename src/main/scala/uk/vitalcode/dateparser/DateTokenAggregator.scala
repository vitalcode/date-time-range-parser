...
object DateTokenAggregator {
  ...
  def aggregate(list: List[DateToken], tp: DateTimeProvider): List[DateToken] = {
    list match {
      ...
      case Day(d, i) :: Month(m, _) :: tail if Date(getYearForNextMonthAndDay(m, d, tp), m, d, i).isSuccess => Date(getYearForNextMonthAndDay(m, d), m, d, i).get :: aggregate(tail, tp)
      case Month(m, i) :: Day(d, _) :: tail if Date(getYearForNextMonthAndDay(m, d, tp), m, d, i).isSuccess => Date(getYearForNextMonthAndDay(m, d), m, d, i).get :: aggregate(tail, tp)

      case Time(from, i) :: Range(_) :: Time(to, _) :: tail => TimeRange(from, to, i) :: aggregate(tail, tp)
