...
class Examples extends ExamplesSupport {
  ...
  override protected val currentDate = LocalDateTime.of(2017, 5, 6, 0, 0)

  override protected val examples = List(
    "6 May 2017" -> List(
      DateTimeInterval.from(2017, 5, 6)
    ),
    "Saturday, January 9th 2017 from 3:00 PM" -> List(
      DateTimeInterval.from(2017, 1, 9, 15, 0)
    ),
    "Thursday, March 17th 2017 from 7:00 PM to 8:30 PM" -> List(
      DateTimeInterval.from(2017, 3, 17, 19, 0).to(2017, 3, 17, 20, 30)
    ),
    "Sept. 10, 2017 12:00pm" -> List(
      DateTimeInterval.from(2017, 9, 10, 12, 0)
    ),
    "Sept. 10, 2017 11:00am, 3:00pm" -> List(
      DateTimeInterval.from(2017, 9, 10, 11, 0),
      DateTimeInterval.from(2017, 9, 10, 15, 0)
    ),
    "Fri 24 Jun 6:45pm (doors) | 11pm (curfew)" -> List(
      DateTimeInterval.from(2017, 6, 24, 18, 45).to(2017, 6, 24, 23, 0)
    ),
    "February, 23 17:25 - 18:05" -> List(
